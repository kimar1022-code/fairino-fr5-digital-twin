...
namespace RobotControl
{
    ...
    public class RobotControlUI : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            // Status 메시지 색상: BUSY면 주황색, 연결되면 초록, 아니면 흰색
            bool isBusy = robotManager.IsBusy;
            if (statusText != null)
            {
                statusText.text = robotManager.StatusMessage;
                if (isBusy) statusText.color = warnColor;
                else statusText.color = robotManager.IsConnected ? readyColor : textColor;
            }
